"""ULCORED system. source: 1. Keys, A., M. Van Hout, and B. Daniels. "Decarbonisation options for the Dutch steel industry." PBL Netherlands Environmental Assessment Agency (2019).
...
def m_iron_fines(m_pellet):
    ...
    n_pellet = (1.13*fe_pellet)/(1.4*fe_iron_fine) #Assume fixed conversion rate in pellet plant
...
def m_flux_pellet(m_pellet):
    cr_flux = (29.79+19.96)*10**-6/1.13 #Assume fixed flux (olivine, limestone) consumption rate. source 1.
...
def m_c_pellet(m_pellet, f_ng, c_coal):
    fe_pellet = 0.635 # Assume fixed value. source 3.
    cr_c_pellet = (0.6038 * 0.01135 + 0.61/49.5*0.7641)/(1.13*0.635) # Assume fixed consumption rate of C =(0.6038 * 0.01135 + 0.64/49.5*0.7641)/(1.4*0.635) , linearly related to Fe kg in pellet
    #f_ng = (0.61/49.5*0.7641)/(0.6038 * 0.01135 + 0.61/49.5*0.7641) =0.590428
    ...
    return c_source
# print(m_c_pellet(1.13,0.5787,0.6038)) #default value. m_ng = 0.01293 kg/kg steel

#calculate amount of electricity needed in pellet plant
def q_elec_pellet(m_pellet, m_coal, m_ng, hhv_coal):
    fe_pellet = 0.635 # Assume fixed value. source 3.
    cr_q_pellet =(0.01135*28.7 + 0.61 + 0.10)/(1.13 * fe_pellet) # =(0.01135*28.7 + 0.64 + 0.11)/(1.4 * fe_pellet)  # Assume fixed energy consumption rate MJ/kg pellet
