...
PATH = os.getcwd() + '/analysis/system/industry/iron_steel/'
...
def user_inputs_ft_cap(cls,cp_options): # if blast furnace, has access to captive power
   return [
      OptionsInput(
         'country', 'Location of plant',
         defaults=[Default('India')],
         options=COUNTRIES
      ),
      InputGroup('elec_options', 'Electricity Inputs', children=[
         ...
         OptionsInput(
            ...
            defaults=[Default('IEA SDS')],
            tooltip=Tooltip('IEA SDS = IEA Sustainable Development Scenario Grid Intensity for Main Grid, User = define grid intensity for 2040, Static = constant grid intensity'),
         ...
         ShareTableInput( # for if the choice is not captive power
            ...
            rows=[
               ShareTableInput.Row(
                  ...
                  cells=[
                     ShareTableInput.Cell(defaults=ci_defaults_base()),
                     ShareTableInput.Cell(defaults=ci_defaults_projected()),
                  ],
               ),
            ],
            on_change_actions=[
         ...
         ContinuousInput(
            ...
            unit='%',
            defaults=[
               Default(35, conditionals=[conditionals.input_equal_to('cp_fuel', 'Coal')]),
               ...
               Default(32, conditionals=[conditionals.input_equal_to('cp_fuel', 'Waste Gas')]),  # see sources in powerplant file
            ],
...
# def user_inputs(cls):
#    # reformat as table
#    # defaults to be replaced based on country
#    return [
#       OptionsInput(
#          'country', 'Location of plant',
#          defaults=[Default('India')],
...
def co2_int(route, years):
   ...
   if elec_source == 'IEA SDS':
      ...
      if grid_source == 'IEA SDS':
         for year in years:
            iea_elec_co2 = ELECTRICITY_DATA[country][
               year]  # elec['co2'][20:25]  # Electricity carbon intensity tonne CO2/GJ.
