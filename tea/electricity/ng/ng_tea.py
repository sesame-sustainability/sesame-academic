...
class NaturalGasTEA(TeaBase):
    ...
    def get_scgt_weight(self):
        filtered = self.filtered_data()
        return float((filtered['SCGT in mix']))  

    def get_ccgt_weight(self):
        filtered = self.filtered_data()
        return float((filtered['CCGT in mix']))  

    def get_ice_weight(self):
        filtered = self.filtered_data()
        return float((filtered['ICE in mix']))  
    ...
    def get_other_costs(self):
        ...
        if "Mix" in turb:
            ...
            adj_turbine_capital = (self.get_scgt_weight()*adj_turbine_capital1 + self.get_ccgt_weight()*adj_turbine_capital2)\
                                  / (self.get_scgt_weight()+self.get_ccgt_weight())  
            ...
            turbine_vom = float(statistics.mean(filtered[filtered['Cost Type'] == "VOM"].value)) 
            turbine_fom = float(statistics.mean(filtered[filtered['Cost Type'] == "FOM"].value)) 
    ...
    def get_cost_breakdown(self):
        ...
        if self.use_CCS == 'Yes':
            ...
            ccs = CcsTea( plant_type = "Natural Gas Power Plants", plant_size = self.plant_size * self.get_capacity_factor(), 
