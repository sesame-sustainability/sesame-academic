...
PATH = os.getcwd() + "/tea/chemical/diesel/"

class DieselTEA(TeaBase):
    unit = '$/MJ'
    ...
    def user_inputs(cls):
        ...
        OptionsInput('state', 'State', options=[state.abbr for state in STATES]+['US average'],
                    ...
                    tooltip=Tooltip(
                    'State selection impacts fuel tax rate (US average 0.6 $/gal) and the cost of raw materials used in the production facility, including crude, hydrogen, electricity, natural gas, …',
                    source='EIA',
    ...
    def __init__(self, lca_pathway=None):
        ...
        self.cost_other = pd.read_csv(PATH + "diesel_costpar.csv")
        self.taxes = pd.read_csv(PATH + "diesel_taxes.csv")
        self.input_fractions = pd.read_csv(PATH + "diesel_input_fractions.csv")
        self.cost_transportation = pd.read_csv(PATH + "diesel_transportation_costs.csv")
    ...
    def get_other_costs(self):
        ...
        diesel_vom = float(filtered[filtered['Item Type'] == "O&M"].iloc[0].value)
        diesel_capital = float(filtered[filtered['Item Type'] == "Capital"].iloc[0].value)
        ...
        return diesel_capital, diesel_vom, taxes, transportation



    def get_cost_breakdown(self):
        diesel_capital, diesel_vom, taxes, transportation = self.get_other_costs()
        #diesel_usage = self.get_amount_diesel()
        crude_cost, other_fuel_cost = self.get_fuel_cost()
        model = SLCOE(crude_cost + other_fuel_cost,
                            diesel_capital, 0, diesel_vom, 1, taxes, transportation)
