...
PATH = os.getcwd() + "/tea/chemical/gasoline/"

class GasolineTEA(TeaBase):
    unit = '$/MJ'
    ...
    def user_inputs(cls):
        ...
        OptionsInput('state', 'State', options=[state.abbr for state in STATES]+['US average'],
                    ...
                    tooltip=Tooltip(
                    'State selection impacts fuel tax rate (US average 0.53 $/gal) and the cost of raw materials used in the production facility, including crude, hydrogen, electricity, natural gas, …',
                    source='EIA',
    ...
    def __init__(self, lca_pathway=None):
        ...
        self.cost_other = pd.read_csv(PATH + "gasoline_costpar.csv")
        self.taxes = pd.read_csv(PATH + "gasoline_taxes.csv")
        self.input_fractions = pd.read_csv(PATH + "gasoline_input_fractions.csv")
        self.cost_transportation = pd.read_csv(PATH + "gasoline_transportation_costs.csv")
    ...
    def get_other_costs(self):
        ...
        return gasoline_capital, gasoline_vom, taxes, transportation



    def get_cost_breakdown(self):
        gasoline_capital, gasoline_vom, taxes, transportation = self.get_other_costs()
